...
DATA_DIR = PROJECT_ROOT / ".tmp_job_offer_smoke"
DATABASE_URL = "sqlite+aiosqlite:///.tmp_job_offer_smoke/cargopt_dev.db"
...
async def exercise_job_offer() -> None:
    ...
    async with session_maker() as session:
        ...
        if len(offers) != 1:
            raise SystemExit(f"expected 1 offer, got {len(offers)}")

        if offers[0].id != offer.id:
            raise SystemExit("offer id mismatch")

        if offers[0].carrier_id != carrier.id:
            raise SystemExit("carrier id mismatch")
...
def main() -> None:
    ...
    os.environ["ENVIRONMENT"] = "job-offer-smoke"
    os.environ["LOG_LEVEL"] = "INFO"

    reset_db()
    run([".venv/bin/alembic", "upgrade", "head"])
    asyncio.run(exercise_job_offer())
    shutil.rmtree(DATA_DIR)

    print("JOB_OFFER_SMOKE_OK")
