...
async def exercise() -> None:
    ...
    async with session_maker() as session:
        ...
        active = await repo.get_active_client_ban(111)
        if active is None:
            raise SystemExit("active ban not found")
        ...
        active_after = await repo.get_active_client_ban(111)
        if active_after is not None:
            raise SystemExit("active ban still present after unban")
